...
class Invoker extends InvokerBase {
  constructor({ serviceName, group, version }) {
    ...
    this[LOCKSIGN] = true;
    this[LOCKQUEUE] = [];
    this[VERSIONTESTER] = new RegExp(`version=${version}`);
  }
  ...
  setProviders(providers) {
    ...
    const pros = providers
      .map((pro) => decodeURIComponent(pro))
      .filter((pro) => this[VERSIONTESTER].test(pro));

    pros.forEach((pro) => {
      const newprovider = new Provider(pro);
      const provider = this[PROVIDERS].get(newprovider.toString());
      if (provider) {
        // 存在,用原有配置覆盖新的provider
        newprovider.merge(provider);
      }
      this[PROVIDERS].set(newprovider.toString(), newprovider);
    });

    Logger.info(`providers ${JSON.stringify(pros)}`);
  ...
  configProviders(configs = []) {
    const confs = configs
      .map((conf) => decodeURIComponent(conf))
      .filter((conf) => this[VERSIONTESTER].test(conf));
    Logger.info(`configs ${JSON.stringify(confs)}`);
    const confsParsed = confs.map((conf) => URL.parse(conf));

    if (confsParsed.length === 0) {
      [...this[PROVIDERS].values()].forEach((provider) => provider.resume());
    } else {
      [...this[PROVIDERS].values()].forEach((provider) => {
        const config = confsParsed.find((conf) => provider.host === conf.host);
        if (config) {
