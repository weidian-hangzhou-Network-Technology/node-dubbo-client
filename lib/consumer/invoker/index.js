...
class Invoker extends InvokerBase {
   ...
   */
  getRegistryPath() {
    let param = Object.assign({ group: this.group, version: this.version }, Config.getDescription());
    let descriptions = `consumer://${Config.ip}/${this.serviceName}?${QS.stringify(param)}`;
  ...
  configProviders(configs = []) {
    let confs = configs.map((conf) => URL.parse(decodeURIComponent(conf)));
    if (confs.length === 0) {
      this[RESUMEPROVIDERS]();
    } else {
      [...this[PROVIDERS].values()].forEach((provider) => {
        let confMatch = confs.filter((conf) => provider.host === conf.host);
        if (confMatch.length > 0) {
          provider.override(QS.parse(confMatch[0].query));
        } else {
          provider.resume();
        }
      });
