...
class Invoker extends InvokerBase {
  ...
  constructor(serviceInfo) {
    ...
    this.serviceName = serviceInfo.service;
    this.group = serviceInfo.group;
    this.version = serviceInfo.version;

    this.providers = new Map();

    this._versionTester = new RegExp(`version=${serviceInfo.version}`);
  ...
  _setupPath() {
    ...
    const desc = `consumer://${config.ip}/${this.group}/${this.serviceName}:${this.version}?${QS.stringify(param)}`;
  ...
  _setProviders(providers) {
    const _providers = providers
      ...
      .filter((item) => this._versionTester.test(item))
