...
class Invoker extends InvokerBase {
  ...
  init() {
    ...
    const descriptions = `${this.protocol}://${this.host}/${this.serviceName}?${QS.stringify(param)}`;
    this.fullPath = `${this.path.provider}/${encodeURIComponent(descriptions)}`;

    registry.publish(this.path.provider, this.fullPath, 'long');
    registry.on(registry.EVENTS.PUBLISH(this.path.provider), () => {
      logger('service published');
    });
  }
